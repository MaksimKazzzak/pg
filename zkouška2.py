...
def convert_to_czk(amount, currency):
    # stáhnu data z webu (v testech se to nahradí mockem)
    response = requests.get(
        "http://www.cnb.cz/cs/financni_trhy/devizovy_trh/kurzy_devizoveho_trhu/denni_kurz.txt"
    )

    # rozdělím text na jednotlivé řádky
    lines = response.text.splitlines()

    # projdu řádky s měnami (první dva řádky přeskočím)
    for line in lines[2:]:
        # rozdělím řádek podle znaku |
        parts = line.split("|")

        # zkontroluji, jestli je to hledaná měna
        if parts[3] == currency:
            # vezmu kurz a množství
            kurz = float(parts[4].replace(",", "."))
            mnozstvi = float(parts[2])

            # spočítám částku v korunách
            vysledek = amount * kurz / mnozstvi
            return round(vysledek, 2)

    # pokud se měna nenašla, vyhodím chybu
