...
# Pokud při procházení seznamu narazíte na cokoli jiného než je číslo (tzn. int nebo float),
# ukončete zpracování seznamu a vraťte dosud vytvořený seznam.
#
...
def process_numbers(numbers):
    # vytvořím prázdný seznam, kam budu ukládat výsledek
    result = []

    # projdu všechny prvky v seznamu
    for x in numbers:
        # pokud prvek není číslo, ukončím zpracování seznamu
        if type(x) not in (int, float):
            break #zruši

        # pokud je číslo větší než 5, tak ho zpracujeme
        if x > 5:
            # přidám do výsledku číslo vynásobené dvěma
            result.append(x * 2)

    # vrátím výsledný seznam
    return result
